...
Create source-backed self-study tutorials, learning maps, glossaries, and roadmaps for any topic. Use this workflow when the user asks to learn, explain, teach, or build a course on a subject.
...
1. Standards, official documentation, laws, specifications, or institution pages
2. Textbooks, peer-reviewed papers, university material, or established technical references
3. Respected field guides, design systems, industry handbooks, or professional organizations
4. Influential practitioner writing, case studies, and widely cited essays
5. Community material only as supporting context, not as the backbone
...
When the topic involves time-sensitive material (laws, standards, APIs, prices, active research), browse or verify current sources. If browsing is unavailable, state which claims are based on existing knowledge and which may need live verification.

### 4. Design the Knowledge Map

Before drafting, make a structure that covers:
...
Tutorial shape: title/promise → orientation → prerequisites → core vocabulary → mental model → main concepts → standard workflow → worked example → common mistakes → practice path → review checklist → source list.
...
If the user requests Word, PDF, slides, spreadsheet, or another file type, use the relevant tool for that format.

For document outputs, include enough structure for navigation: title, short orientation, sections, glossary, practice or review section, and source list.
